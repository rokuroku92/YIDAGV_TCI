...
function init() {
  ...
  let ctx2 = document.getElementById("batteryTaskChart");
  batteryTaskChart = new Chart(ctx2, {
    type: "line",
    data: {
      labels: [],
      datasets: [
  ...
  xhr.onload = function () {
    if (xhr.status == 200) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        html +=
          '<li><a class="dropdown-item" onclick="reSet(' +
          yyyy +
          mm +
          ')">' +
          data[i].year +
      ...
      html +=
        '<li><a class="dropdown-item" onclick="reSet(\'all\')">全部</a></li>';
...
function renderChart(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    myChart.data.datasets[0].data.push(
      ((data[i].workingMinute / 1440) * 100).toString().substring(0, 2)
    );
    ...
    html +=
      ...
      data[i].task +
      "</td><td>" +
      String((data[i].workingMinute / 1440) * 100).substring(0, 2) +
      "%</td><td>" +
      Math.floor(data[i].workingMinute / 60) +
      "</td><td>" +
      Math.floor(data[i].openMinute / 60) +
      "</td>" +
      '<td><input class="hh" data-wd="' +
      datawd +
      '" type="checkbox" id="' +
      data[i].month.toString() +
      data[i].day.toString() +
  ...
  document.getElementById("task").value = String(
    (task_sum / x) * 100
  ).substring(0, 2);
  document.getElementById("open_sum").value =
    String(Math.floor(open_sum / 60)) + "hr";
  document.getElementById("work_sum").value =
    String(Math.floor(work_sum / 60)) + "hr";
  document.getElementById("work").value =
    String((Math.floor(work_sum / 60) / x).toFixed(1)) + "hr";
  document.getElementById("rate").value =
    String((work_sum / 1440 / x) * 100).substring(0, 2) + "%";
  ...
  $(function () {
    ...
    $("#cancelweekend").click(function (event) {
      ...
      $("#checkAll").prop(
        "checked",
        tbr.find("input[type=checkbox]:checked").length == tbrr.length
          ? true
          : false
      );
      // 阻止向上冒泡，以防再次觸發點擊操作
...
function printOut() {
  ...
  var llen = 0;
  console.log(len);
  for (let i = 0; i < len; i++) if (table.rows[i].innerHTML != "") llen++;
  console.log(llen);
  var task_sum = 0;
  var rate_sum = 0;
  var work_sum = 0;
  ...
  for (let i = 0; i < len; i++) {
    if (table.rows[i].innerHTML != "") {
      ...
      rate_sum += Number(
        String(table.rows[i].cells[2].innerHTML).replace("%", "")
      );
  ...
  document.getElementById("work").value =
    String(work_sum / len).substring(0, 4) + "hr";
  document.getElementById("rate").value =
    String(Math.round(rate_sum / len)).substring(0, 2) + "%";
  window.print();
...
function getbatteryTask() {
  var xhr = new XMLHttpRequest();
  xhr.open(
    "GET",
    baseUrl + "/api/analysis/batteryTask?agvId=1&date=recently",
    true
  );
  xhr.send();
  xhr.onload = function () {
    if (xhr.status == 200) {
      let data = JSON.parse(this.responseText);
      console.log(data);
      renderbatteryTaskChart(data);
    }
  };
}

function renderbatteryTaskChart(data) {
  batteryTaskChart.data.labels = [];
  batteryTaskChart.data.datasets[0].data = [];
  batteryTaskChart.data.datasets[1].data = [];
  for (let i = 0; i < data.length; i++) {
    batteryTaskChart.data.labels.push(data[i].date);
    batteryTaskChart.data.datasets[0].data.push(data[i].battery);
    batteryTaskChart.data.datasets[1].data.push(data[i].task);
  }
  batteryTaskChart.update();
}

async function refreshBatteryTaskData() {
  const selectDate = document.getElementById('selectDate').value.replaceAll("-","");
  const response = await fetch(`${baseUrl}/api/analysis/batteryTask?agvId=1&date=${selectDate}`);
